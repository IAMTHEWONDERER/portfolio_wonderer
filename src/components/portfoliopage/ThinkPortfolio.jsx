      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden">
          {thinkingPhases.map((phase, index) => (
            <div
              key={index}
              className={`absolute w-64 h-64 bg-gradient-to-br ${phase.color} opacity-[0.02] rounded-full blur-3xl transition-all duration-1000 ${activePhase === index ? 'scale-110 opacity-[0.04]' : 'scale-90 opacity-[0.01]'
                }`}
              style={{
                left: `${10 + index * 18}%`,
                top: `${20 + (index % 2) * 40}%`,
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 max-w-5xl mx-auto">
                {thinkingPhases.slice(0, 3).map((phase, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`bg-white border transition-all duration-700 overflow-hidden h-80 flex flex-col ${isActive
                        ? 'border-[#e61f00] shadow-xl'
                        : 'border-[#0a0100]/10 hover:border-[#0a0100]/20 hover:shadow-lg'
                        }`}>

                        {/* Card Header */}
                        <div className={`p-6 flex-1 flex flex-col transition-all duration-700 ${isActive ? 'bg-gradient-to-br from-[#e61f00]/5 via-transparent to-[#e61f00]/10' : ''
                          }`}>
                          ...
                          <div className="flex items-center justify-between mb-4">
                            ...
                            <div className={`w-6 h-6 rounded-full border-2 transition-all duration-500 flex items-center justify-center ${isActive
                              ? 'border-[#e61f00] bg-[#e61f00]'
                              : 'border-[#0a0100]/20 group-hover:border-[#e61f00]/50'
                              }`}>
                              <Circle className={`w-2 h-2 transition-colors duration-500 ${isActive ? 'text-white' : 'text-transparent'
                                }`} />
                            </div>
                          </div>

                          {/* Icon */}
                          <div className={`w-14 h-14 flex items-center justify-center mb-4 transition-all duration-500 ${isActive
                          ...
                          {/* Title */}
                          <h4 className={`font-erstoria text-lg tracking-wide mb-2 transition-colors duration-300 ${isActive ? 'text-[#e61f00]' : 'text-[#0a0100] group-hover:text-[#e61f00]'
                            }`}>
                            {phase.title}
                          </h4>
              ...
              <div className="flex gap-3">
                {thinkingPhases.map((_, index) => (
                  <button
                    key={index}
                    onClick={() => setActivePhase(index)}
                    className={`relative group transition-all duration-300 cursor-pointer ${index === activePhase
                      ? 'scale-110'
                      : 'hover:scale-105'
                      }`}
                  >
                    <div className={`w-4 h-4 rounded-full transition-all duration-300 ${index === activePhase
